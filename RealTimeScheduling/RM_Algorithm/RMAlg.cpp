...
struct processInfo{
    ...
    //The < operator decides which process is placed into the queue based on priority requirement. In the case of RMA, the priority of a process is inversely proportional to the period it will run for.
    bool operator<(const processInfo &o) const
        {
            return (1/period) < (1/o.period);
        }
};
