...
struct processInfo{
    ...
    //The < operator decides which process is placed into the queue based on priority requirement. In the case of EDF, the priority of a process is based on the earliest deadline.
    bool operator<(const processInfo &o) const
        {
            return upcoming_deadline> o.upcoming_deadline;
        }
};
